# 用GitHub+Actions+Render白嫖一套金融数据中心

——从Excel周报到免费API数据系统的工程化实践

## 0.开始的开始

后来回头看，我发现这个项目真正的核心，其实不是“API”。

更准确地说，它是在做一件更基础的事：把数据层独立出来。

最开始我只是想做一套自己的市场周报系统。Excel里有PowerQuery，有图表，有宏，有一堆从不同网站扒下来的数据源。刚开始这套东西很好用，xlsm点一下刷新，图表更新，周报也能继续写。

但数据源越来越多之后，问题开始变得很明显：

- 每次做周报都要重新请求所有数据源。
- 某个接口慢了，整个Excel都卡住。
- 某个网页改了字段，PowerQuery就炸。
- 历史数据反复下载，完全没有必要。
- 周报、图表、分析逻辑和数据获取绑得太死。
...
flowchart LR
    ...
    B-->C[GitHub仓库中的数据中心]
    C-->D[FastAPI服务]
    D-->E[Excel和PowerQuery]
    D-->F[API展示页]
    D-->G[周报和分析系统]
    C-->H[GitHubActions自动更新]
```

【图1：旧版Excel/PowerQuery工作流截图，展示多个查询直接连外部数据源的状态】
...
所以这篇文章里说的API，本质上就是一组稳定的URL。Excel、网页、Python脚本都可以访问它们，并拿到结构化数据。
...
【图2：PowerQuery查询编辑器截图，展示早期直接在Excel里请求外部数据的状态】
...
那时候系统其实已经出现了一个信号：我不是缺数据库能力，而是不应该让整个项目围绕一个越来越大的中心数据库旋转。

如果继续走“大型单体SQLite”路线，短期还能用，但长期一定会越来越难维护。这个项目需要的不是一个更重的数据库，而是一个更适合GitHub、自动提交、增量更新和API读取的数据仓库结构。

这也是后来转向Parquet分片的原因。

【图3：旧SQLite文件或LFS提示截图，展示单体数据库开始变重的阶段】
...
```mermaid
flowchart TD
    A[GitHub仓库]-->B[保存代码和数据文件]
    A-->C[版本管理和历史追踪]
    D[GitHubActions]-->E[定时运行fetcher]
    E-->F[更新Parquet和日志]
    F-->G[自动commit并push]
    H[Render]-->I[运行FastAPI]
    I-->J[api.huangdapao.com]
    K[GitHubPages]-->L[API展示页]
    M[飞书机器人]-->N[推送运行结果]
```
...
这个设计的核心不是无限扩张，而是在免费资源限制内尽量优化结构。
...
【图4：GitHubActions一次成功运行截图，展示Runfetcherbatch、Commitupdateddataandlogs、飞书通知等步骤】
...
```mermaid
flowchart LR
    A[GitHub仓库中的api/data]-->B[Render部署api目录]
    B-->C[FastAPI应用main.py]
    C-->D[各retours路由模块]
    D-->E[DuckDB读取Parquet]
    D-->F[SQLite兼容读取]
    E-->G[JSON响应]
...
【图5：Render服务部署页面截图，展示启动命令、环境变量和部署状态】

【图6：Dockerfile截图，展示API目录被复制进容器并安装requirements】
...
【图7：飞书机器人推送截图，展示一次自动更新后的成功/失败摘要】
...
但回到这个项目本身，我后来发现它真正需要的并不是一个复杂的中心后端。它的数据访问量不高，用户主要是我自己的Excel、API展示页、后续周报脚本和少量浏览器访问。它的核心压力也不是高并发，而是长期维护、低成本更新、历史数据稳定保存和随时可追踪。

所以这套系统最后没有继续往“MySQL+Redis+Nginx+云服务器”的方向走，而是变成了几个轻量组件的组合：

```mermaid
flowchart TD
    A[GitHub仓库]-->B[Parquet数据文件]
    F[GitHubActions]-->B
    B-->C[DuckDB查询层]
    C-->D[FastAPI接口层]
    D-->E[Render API服务]
    G[GitHubPages]-->H[API展示页]
    E-->H
```
...
```mermaid
flowchart LR
    A[用户访问api.huangdapao.com]-->B[Render接收公网请求]
    B-->C[Uvicorn监听端口]
    C-->D[FastAPI应用main:app]
    D-->E[匹配API路由]
    E-->F[调用retours查询模块]
    F-->G[DuckDB读取Parquet]
    G-->H[FastAPI返回JSON或CSV]
```
...
```mermaid
flowchart LR
    A[GitHub仓库中的Parquet数据]-->B[FastAPI服务]
    B-->C[JSON API]
    B-->D[CSV下载]
    B-->E[API展示页]
    F[SQLite兼容数据]-->B
...
```mermaid
flowchart TD
    A[用户请求/data?startdate=...&format=json]-->B[FastAPI路由]
    B-->C[解析参数和默认值]
    C-->D[根据日期定位月份Parquet]
    D-->E[DuckDB read_parquet查询]
    E-->F[字段标准化和中文列名meta]
    F-->G{format参数}
    G-->|json|H[返回JSON]
    G-->|csv|I[返回CSV文件]
...
【图8：FastAPI自动生成docs页面截图，展示路由列表和/health接口】

【图9：API返回JSON示例截图，展示meta、columns_zh和data结构】
...
```mermaid
flowchart LR
    A[用户访问/shibor_curve/data<br/>startdate=2025-01-01<br/>enddate=2025-03-31<br/>format=json]
    -->B[Render转发请求]
    -->C[FastAPI匹配/shibor_curve/data路由]
    -->D[解析startdate/enddate/limit/format]
    -->E[定位202501-202503相关Parquet切片]
    -->F[DuckDB执行read_parquet查询]
    -->G[按日期过滤和排序]
    -->H[组装meta和data]
    -->I[返回JSON]
```
...
```mermaid
flowchart TD
    A[Parquet按月切片]-->B[DuckDB按需查询]
    B-->C[FastAPI统一响应]
    C-->D{请求格式}
    D-->|format=json|E[返回JSON给页面/Excel/脚本]
    D-->|format=csv|F[返回CSV下载]
    E-->G[API页面渲染前五条预览]
```

这也是这套系统后来变轻的原因：它没有依赖一个巨大的中心服务器来处理所有事情，而是让几个小组件在正确的位置上各自完成一小段工作。
...
【图10：融资融券合并API页面截图，展示两市字段和CSV按钮】
...
【图11：ChromeDevToolsNetwork截图，展示YahooFinanceChart请求地址和参数】
...
【图12：新发基金API返回结果截图，展示pending基金排在前面】
...
```mermaid
flowchart TD
    A[打开网页]-->B[看ChromeDevToolsNetwork]
    B-->C[定位真实数据请求]
    C-->D[复制URL和headers]
    D-->E[用Pythonrequests复现]
    E-->F[处理timeout/retry/TLS/JSONP]
    F-->G[标准化字段]
    G-->H[写入Parquet]
    H-->I[接入FastAPI]
    I-->J[进入API展示页]
...
【图13：ChromeDevToolsNetwork面板截图，展示东方财富或上交所真实请求地址】
...
```mermaid
flowchart TD
    A[GitHubActions定时触发]-->B[checkout仓库]
    B-->C[安装Python依赖]
    C-->D[运行run_all_fetchers.py]
    D-->E[逐个运行fetcher]
    E-->F[更新Parquet或SQLite]
    F-->G[追加fetcher_daily_runs.csv]
    G-->H[更新api_showcase metadata]
    H-->I[git add api/data和metadata和日志]
    I-->J[git commit并push]
    J-->K[飞书机器人推送摘要]
```
...
【图14：fetcher_daily_runs.csv截图，展示一次运行中每个脚本的状态和耗时】
...
```mermaid
flowchart TD
    A[用户打开API页面]-->B[读取metadata目录]
    B-->C[展示分类和接口卡片]
    C-->D[用户填写参数]
    D-->E[页面生成请求URL]
    E-->F{用户选择操作}
    F-->|复制链接|G[复制API URL]
    F-->|打开JSON|H[访问FastAPI JSON接口]
    F-->|下载CSV|I[访问FastAPI CSV接口]
    F-->|预览数据|J[请求前五条并渲染表格]
```
...
【图15：API首页截图，展示两级目录、API域名、可用接口和搜索框】

【图16：单个API详情截图，展示参数输入、字段说明、JSON链接和CSV下载按钮】

【图17：CSV下载按钮截图，展示format=csv生成的下载入口】

【图18：点击请求并预览后，表格展示前五条数据的截图】
...
```mermaid
flowchart TD
    A[采集层fetcher]-->B[存储层Parquet/SQLite]
    B-->C[查询层FastAPI]
    C-->D[展示层api_showcase]
    C-->E[消费层Excel/PowerQuery]
    A-->F[调度层GitHubActions]
    F-->G[通知层飞书机器人]
```
...
这就是项目从“脚本集合”变成“数据系统”的关键。每个新数据源进入系统后，不只是多了一个文件，而是同时进入采集、存储、查询、展示、自动化和通知这条链路。
